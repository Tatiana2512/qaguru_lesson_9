...
В первом случае результатом поиска элемента по локатору будет ошибка, поскольку для коллекций необходимо применять метод `$$` (в случае, если следом нет методов, явно указывающих на один элемент из коллекции). Во втором случае будет выбран блок `<div>`, содержащий ссылку.
